...
class Controller extends AttributeTypeController
{
    ...
    public function filterByAttribute(AttributedItemList $list, $value, $comparison = '=')
    {
        ...
        /** Darum Patched **/
        if ($comparison == 'by_slug' && is_string($value)) {
            $qb = $list->getQueryObject();
            $qb->join('cv', 'CollectionAttributeValues', 'cav', 'cav.cvID = cv.cvID');
            $qb->join('cav','atSelectOptionsSelected', 'atsos', 'atsos.avID = cav.avID');
            $qb->join('atsos', 'atSelectOptions', 'aso', 'atsos.avSelectOptionID = aso.avSelectOptionID');
            $qb->andWhere("aso.slug=:slug");
            $qb->andWhere("cav.cID = p.cID");
            $qb->andWhere("aso.isDeleted = 0");
    ...
    public function saveKey($data)
    {
        ...
        if (isset($data['akSelectGenerateSlug']) && ($data['akSelectGenerateSlug'] == 1)) {
            $akSelectGenerateSlug = 1;
        } else {
            $akSelectGenerateSlut = 0;
        }
